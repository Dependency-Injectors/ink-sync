  ...
  useEffect(() => {
    const fetchImage = async () => {
      ...
      try {
        ...
        const isAxiosError = (error: unknown): error is { response?: { status?: number; data?: { error?: string } }; message?: string } => {
        ...
        if (statusCode === 401) {
          setError('Authentication required (401): Please log in to access this image');
        } else if (statusCode === 404) {
          setError('Image not found (404): This image does not exist or you do not have access');
        } else {
      ...
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gray-900 ml-16 gap-4">
        ...
        {error?.includes('401') || error?.includes('Unauthorized') ? (
          <div className="flex flex-col items-center gap-2">
            <p className="text-gray-400">Please log in to access this drawing room</p>
            <a 
              href="/login" 
